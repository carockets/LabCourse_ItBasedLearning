...
listtext0 = [range (100 , 150 ), range(100,201), (1,1) ]  #  1,1
...
def change_graphic(x,y):
    ...
    if test in remaining_coins:
        ...
        if level[x][y] == "4":
            ...
            #sound_flag.play() # will be interrupted when changing windows. Moved to other new window.
        else:
            st3 = st2 + "."
            level[x] = st3 + st1[y+1:len(level[0])]
...
# Improved Coin collecting part. Smaller size and easier to handle.
def listcoins(playerleft, playerbottom):
    ...
    if playerleft in listtext0[0]:
        if playerbottom in listtext0[1]:
...
showText2 = True # Set True for Coin Counter to be shown
...
# Background music
pygame.mixer.music.load('bgSound1.ogg')
...
while True:
    ...
    if playerSpeedX == 0:
        1+1
        # Does nothing, as player doesnt move
        #  lastDir remains set to the previous value.
    else:
        if playerSpeedX < 0:
            lastDir = 'left'
        else:
...
#///////////////////////Rendering///////////////////////#
    ...
    enemymove()
    if enemyDir_L == True:
        screen.blit(myimage_enemy, ((14*BLOCKWIDTH)-camerax-enemy_movement, (10*BLOCKHEIGHT)-cameray, BLOCKWIDTH, BLOCKHEIGHT))
    else:
        screen.blit(myimage_enemy, ((14*BLOCKWIDTH)-camerax-enemy_movement, (10*BLOCKHEIGHT)-cameray, BLOCKWIDTH, BLOCKHEIGHT))
    ...
    if showText2:
        drawText('Muenzen: %r' %(coin_counter), font, WHITE, screen, 10, 0)
...
# Button Rendering #
    button("Music", 730,0,70,30 ,GREEN, LIGHTGREEN , "music")
...
# Left side collision detection with an enemy
    if (player.left - camerax+20) in range ((14*BLOCKWIDTH)-camerax-enemy_movement, (14*BLOCKWIDTH)-camerax-enemy_movement+100):
        if (player.top - cameray) in range ((10*BLOCKHEIGHT)-cameray,(10*BLOCKHEIGHT)-cameray+100 ):
            playerHP -= 1
            if playerHP == 0:
                dying()
            else:
    ...
    # enemy_movement is just for one single enemy at the moment.
    # if we implement more than one enemy we will have to copy this and
    # adapt the varibles in front of BLOCKWIDTH/BLOCKHEIGHT to the
    # specific enemy.
    if (player.right - camerax-20) in range ((14*BLOCKWIDTH)-camerax-enemy_movement, (14*BLOCKWIDTH)-camerax-enemy_movement+100):
        if (player.top - cameray) in range ((10*BLOCKHEIGHT)-cameray,(10*BLOCKHEIGHT)-cameray+100 ):
            playerHP -= 1
            if playerHP == 0:
                dying()
    ...
    if playerHP == 1:
        screen.blit(myimage_hp, pygame.rect.Rect(210,0, 37,33 ))
    if playerHP == 2:
        screen.blit(myimage_hp, pygame.rect.Rect(210,0, 37,33 ))
        screen.blit(myimage_hp, pygame.rect.Rect(260,0, 37,33 ))
    if playerHP == 3:
        screen.blit(myimage_hp, pygame.rect.Rect(210,0, 37,33 ))
        screen.blit(myimage_hp, pygame.rect.Rect(260,0, 37,33 ))
        screen.blit(myimage_hp, pygame.rect.Rect(310,0, 37,33 ))
